...
The H3C GR-1200W (<=MiniGRW1A0V100R006)  router was found to have a stack overflow vulnerability in the UpdateDDNS function. An attacker can obtain a stable root shell through a carefully constructed payload.

![image-20220724213415656](img/image-20220724213415656.png)

In the `UpdateDDNS` function, the `param` we entered is formatted using the `sscanf` function and in the form of `%s`. This greedy matching mechanism is not secure, as long as the size of the data we enter is larger than the size of `V11`, it will cause a stack overflow.
...
CMD=UpdateDDNS&param=;
```
